...
**Active: Phase 1 + Phase 4 + Phase 5 + Phase 6 + Phase 7 + Phase 9 + Gas Town Sprint 1 + lean-canvas + /scaffold-frame-app + SEH Study + fleet-wide security scanning + fleet-wide dependency hardening + skill-catalog + fleet-wide session-init + beaverGame + asset-foundry + github-actions (shared CI) + selfco vault runner + OPAV instrumentation + manifest-driven domain decomposition**
