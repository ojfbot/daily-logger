...
**`GET /api/tools` threat model:** The endpoint is static JSON, no auth, no rate limiting. The assumption is internal network / development context only — Frame is not deployed to a public URL. This is running locally. The "discovery only" framing is accurate for that context, but if the deployment scope changes, the threat model needs to be revisited explicitly before the endpoint stays as-is.
...
**Frame infrastructure cost:** Frame is running locally — not deployed to a production environment. There is no current monthly infrastructure number to surface. When deployment moves beyond localhost, that number needs to be tracked explicitly.
